...
class DualAttentionNetwork(nn.Module):
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx):
        ...
        # m_out = self._maybe_mask_zero_(m_out, mch_mask.diagonal(dim1=1, dim2=2), M)
...
class DANIEL(nn.Module):
    ...
    def forward(
        ...
        dynamic_pair_mask,
        fea_pairs,
        action_mask: torch.Tensor = None,   # [ADDED] 額外的 action mask（J_max * M 維）
    ):
        """
        ...
        :param action_mask: [ADDED] 形狀 [sz_b, J*M] 或 [sz_b, J, M] 的布林遮罩；
                            False 代表「禁止選擇」（例如 dummy job）。
        :return:
        ...
        candidate_scores = candidate_scores.squeeze(-1)  # [sz_b, J*M]

        # ---- 1) 先用 dynamic_pair_mask 擋掉不相容 op-mch ----
        if dynamic_pair_mask is not None:
            # dynamic_pair_mask: [sz_b, J, M] → reshape 成 [sz_b, J*M]
            dyn_mask = dynamic_pair_mask.reshape(sz_b, -1).to(torch.bool)
            # True = 不可用 → -inf
            candidate_scores = candidate_scores.masked_fill(dyn_mask, float('-inf'))

        # ---- 2) 再用 action_mask 擋掉 dummy job（J_max padding 部分）[ADDED] ----
        if action_mask is not None:
            # action_mask 可是 [sz_b, J*M] 或 [sz_b, J, M]
            if action_mask.dim() == 3:
                act_mask = action_mask.reshape(sz_b, -1).to(torch.bool)
            else:
                act_mask = action_mask.to(torch.bool)
            if act_mask.shape != candidate_scores.shape:
                raise ValueError(
                    f"action_mask shape {act_mask.shape} 與 candidate_scores {candidate_scores.shape} 不符"
                )
            # False = 禁用 → -inf
            candidate_scores = candidate_scores.masked_fill(~act_mask, float('-inf'))
