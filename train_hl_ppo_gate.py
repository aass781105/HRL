...
def collect_episode(
    ...
    release_bias_sum = 0.0
    release_bias_count = 0
    info = {}
    while not done:
        ...
        with torch.no_grad():
            ...
            action = dist.sample()
            forced_last_release = float(obs[-1]) >= 0.5
            if forced_last_release:
                action = torch.ones_like(action, dtype=torch.int64)
            logp = dist.log_prob(action)
        act = int(action.item())
        if act == 1 and not forced_last_release:
            probs = torch.softmax(logits, dim=1)
            release_bias_sum += float((probs[0, 1] - probs[0, 0]).item())
    ...
    return {
        ...
        "release_bias_sum": float(release_bias_sum),
        "release_bias_count": int(release_bias_count),
    }
...
def collect_episode_batch_vectorized(
    ...
    ep_release_bias_sum = np.zeros(num_envs, dtype=np.float32)
    ep_release_bias_count = np.zeros(num_envs, dtype=np.int64)
    ...
    while not np.all(completed):
        ...
        release_biases = np.zeros(num_envs, dtype=np.float32)
        forced_last_mask = np.zeros(num_envs, dtype=np.bool_)
        ...
        if active_idx.size > 0:
            ...
            release_biases[active_idx] = (probs[:, 1] - probs[:, 0]).cpu().numpy()
            forced_last_mask[active_idx] = last_mask.cpu().numpy()
        ...
        for idx in active_idx:
            ...
            if int(actions[idx]) == 1 and not bool(forced_last_mask[idx]):
                ep_release_bias_sum[idx] += float(release_biases[idx])
                ep_release_bias_count[idx] += 1
    ...
    for idx in range(num_envs):
        ...
        episodes.append({
            ...
            "release_bias_sum": float(ep_release_bias_sum[idx]),
            "release_bias_count": int(ep_release_bias_count[idx]),
        })
    return episodes
...
def merge_episode_batch(
    ...
    release_decay = float(getattr(configs, "hl_release_reward_decay", 0.9))
    ...
    total_release_bias_count = int(sum(int(ep.get("release_bias_count", 0)) for ep in episodes))
    total_release_bias_sum = float(sum(float(ep.get("release_bias_sum", 0.0)) for ep in episodes))
    summary = {
        ...
        "release_bias": float(total_release_bias_sum / total_release_bias_count) if total_release_bias_count else 0.0,
        "release_bias_count": total_release_bias_count,
...
def update_ppo(
    ...
    raw_adv_t = torch.as_tensor(advantages, dtype=torch.float32, device=device)
    ret_t = torch.as_tensor(returns, dtype=torch.float32, device=device)
    with torch.no_grad():
        _, value_before = model(obs_t)
        ret_var = torch.var(ret_t, unbiased=False)
        explained_var = 1.0 - torch.var(ret_t - value_before, unbiased=False) / (ret_var + 1e-8)
        raw_adv_mean = float(raw_adv_t.mean().item())
        raw_adv_std = float(raw_adv_t.std(unbiased=False).item())
        raw_adv_pos = float((raw_adv_t > 0).float().mean().item())
        return_std = float(ret_t.std(unbiased=False).item())
        value_std = float(value_before.std(unbiased=False).item())
    adv_t = (raw_adv_t - raw_adv_t.mean()) / (raw_adv_t.std(unbiased=False) + 1e-8)

    n = obs_t.shape[0]
    losses, p_losses, v_losses, entropies, approx_kls, clip_fracs = [], [], [], [], [], []
    ...
    return {
        ...
        "raw_adv_pos": raw_adv_pos,
        "return_std": return_std,
        "value_std": value_std,
    }


def eval_same_problem_greedy(model: HLPPOGateNet, device: torch.device, eval_seed: int) -> Tuple[float, float, int]:
    env = build_env()
...
def validate_greedy(model: HLPPOGateNet, device: torch.device, seed: int = 999) -> Tuple[float, float, int]:
    env = build_env()
...
def train_hl_ppo_gate():
    start_ts = time.time()
    seed = int(getattr(configs, "hl_ppo_train_seed", getattr(configs, "event_seed", 42)))
    random.seed(seed)
    np.random.seed(seed)
    ...
    lr_start = float(getattr(configs, "hl_ppo_lr", 1e-4))
    lr_end = float(getattr(configs, "hl_ppo_lr_end", 5e-5))
    lr_decay = bool(getattr(configs, "hl_ppo_lr_decay", True))
    gamma = float(getattr(configs, "hl_ppo_gamma", 0.99))
    gae_lambda = float(getattr(configs, "hl_ppo_gae_lambda", 0.95))
    clip_ratio = float(getattr(configs, "hl_ppo_clip", 0.2))
    value_coef = float(getattr(configs, "hl_ppo_value_coef", 0.5))
    entropy_start = float(getattr(configs, "hl_ppo_entropy_coef", 0.01))
    entropy_end = float(getattr(configs, "hl_ppo_entropy_end", entropy_start))
    entropy_decay = bool(getattr(configs, "hl_ppo_entropy_decay", False))
    update_epochs = int(getattr(configs, "hl_ppo_update_epochs", 5))
    minibatch_size = int(getattr(configs, "hl_ppo_minibatch_size", 64))
    max_grad_norm = float(getattr(configs, "hl_ppo_max_grad_norm", 1.0))
    num_envs = max(1, int(getattr(configs, "hl_ppo_num_envs", 1)))
    use_async_envs = bool(getattr(configs, "hl_ppo_use_async_envs", True))
    same_problem_eval_every = int(getattr(configs, "hl_ppo_same_problem_eval_every", 0))
    validate_every = int(getattr(configs, "hl_ppo_validate_every", 10))
    episodes_per_instance = max(
        1,
        int(getattr(configs, "hl_ppo_instance_episodes", 10)),
    )
    ...
    history_raw_adv_mean, history_raw_adv_std, history_raw_adv_pos = [], [], []
    history_return_std, history_value_std = [], []
    history_lr, history_release_bias, history_release_bias_count = [], [], []
    ...
    for update_idx in range(1, updates + 1):
        ...
        history_kl.append(float(upd["approx_kl"]))
        history_clip.append(float(upd["clip_frac"]))
        history_ev.append(float(upd["explained_var"]))
        history_raw_adv_mean.append(float(upd["raw_adv_mean"]))
        history_raw_adv_std.append(float(upd["raw_adv_std"]))
        history_raw_adv_pos.append(float(upd["raw_adv_pos"]))
        history_return_std.append(float(upd["return_std"]))
        history_value_std.append(float(upd["value_std"]))
        history_lr.append(float(lr_now))
        history_release_bias.append(float(ep["release_bias"]))
        history_release_bias_count.append(int(ep["release_bias_count"]))
        ...
        line_parts = [
            ...
            f"ReleaseBias={history_release_bias[-1]:7.3f}",
            f"RelSamples={history_release_bias_count[-1]:4d}",
            f"KL={history_kl[-1]:7.4f}",
            f"Clip={history_clip[-1] * 100.0:5.1f}%",
    ...
    with open(train_log_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Update", "Return", "Loss", "Policy_Loss", "Value_Loss", "Entropy", "Approx_KL", "Clip_Frac", "Explained_Var",
            "Raw_Adv_Mean", "Raw_Adv_Std", "Raw_Adv_Pos_Ratio", "Return_Std", "Value_Std",
            "LR", "Release_Bias", "Release_Bias_Samples",
        ...
        for i in range(len(history_ret)):
            writer.writerow([
                i + 1, history_ret[i], history_loss[i], history_ploss[i], history_vloss[i], history_entropy[i],
                history_kl[i], history_clip[i], history_ev[i],
                history_raw_adv_mean[i], history_raw_adv_std[i], history_raw_adv_pos[i], history_return_std[i], history_value_std[i],
                history_lr[i], history_release_bias[i], history_release_bias_count[i],
    ...
    with open(os.path.join(plot_dir, f"time_{name}.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"hl_ppo_lr={lr_start}\n")
        f.write(f"hl_ppo_lr_end={lr_end}\n")
        f.write(f"hl_ppo_gamma={gamma}\n")
        f.write(f"hl_ppo_gae_lambda={gae_lambda}\n")
        f.write(f"hl_ppo_clip={clip_ratio}\n")
        f.write(f"hl_ppo_entropy_coef={entropy_start}\n")
        f.write(f"hl_ppo_entropy_decay={entropy_decay}\n")
        f.write(f"hl_ppo_entropy_end={entropy_end}\n")
        f.write(f"hl_ppo_value_coef={value_coef}\n")
        f.write(f"hl_ppo_update_epochs={update_epochs}\n")
        f.write(f"hl_ppo_minibatch_size={minibatch_size}\n")
        f.write(f"hl_ppo_num_envs={num_envs}\n")
        f.write(f"hl_ppo_use_async_envs={use_async_envs}\n")
        f.write(f"hl_ppo_instance_episodes={episodes_per_instance}\n")

    train_time_path = os.path.abspath("train_time.txt")
    with open(train_time_path, "a", encoding="utf-8") as f:
        f.write("\n")
        f.write("========== HL PPO Gate Training ==========\n")
        f.write(f"hl_ppo_name={name}\n")
        f.write(f"config_path={getattr(configs, 'config', '')}\n")
        f.write(f"start_time={time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_ts))}\n")
        f.write(f"end_time={time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}\n")
        f.write(f"train_seconds={elapsed:.3f}\n")
        f.write(f"train_minutes={elapsed / 60.0:.3f}\n")
        f.write(f"train_hours={elapsed / 3600.0:.3f}\n")
        f.write(f"updates={updates}\n")
        f.write(f"hl_ppo_num_envs={num_envs}\n")
        f.write(f"hl_ppo_use_async_envs={use_async_envs}\n")
        f.write(f"hl_ppo_instance_episodes={episodes_per_instance}\n")
        f.write(f"avg_seconds_per_update={elapsed / max(1, updates):.3f}\n")
