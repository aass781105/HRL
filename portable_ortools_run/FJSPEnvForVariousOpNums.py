...
class FJSPEnvForVariousOpNums:
    ...
    def construct_pair_features(self):
        ...
        true_schedule_matrix = ma.array(true_pair_free_time, mask=self.candidate_process_relation)
        true_next_schedule_time = np.min(
            true_schedule_matrix.reshape(self.number_of_envs, -1), axis=1
        ).data

        pair_wait_time = np.maximum(0.0, true_pair_free_time - true_next_schedule_time[:, np.newaxis, np.newaxis])
        ...
        self.fea_pairs = np.stack((self.candidate_pt,
                                   ...
                                   self._log1p_scaled(pair_wait_time),
