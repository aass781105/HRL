...
def validate_unique_job_ids(payload: Dict) -> None:
...
def run_event_driven_ortools_cadence(
    *,
    ...
    def compute_global_kpis(orchestrator: GlobalTimelineOrchestrator) -> Dict[str, float]:
        ...
        if getattr(orchestrator, "machine_free_time", None) is not None and len(orchestrator.machine_free_time) > 0:
            max_end = max(max_end, float(np.max(orchestrator.machine_free_time)))
        total_td = sum(
            max(0.0, float(job_finish.get(int(job_id), 0.0)) - float(due))
            for job_id, due in all_job_due_dates.items()
        )
        return {
