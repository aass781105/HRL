...
> 5. **主動詢問優先原則 (Ask Before Action)**：任何時候若對使用者的需求、設定檔結構、參數命名、修改方向或執行命令有任何不確定，**絕對必須先向使用者發問確認，禁止擅自猜測、搜尋、新建檔案或修改程式碼**。以後如果遇到任何不確定的問題，必須立刻詢問，禁止擅自行動。一切後續行為均須嚴格遵循此手冊規範。
