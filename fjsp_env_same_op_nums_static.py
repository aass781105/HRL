...
class FJSPEnvForSameOpNums:
    ...
    def set_static_properties(self):
        ...
        """
        self.multi_env_mch_diag = np.tile(np.expand_dims(np.eye(self.number_of_machines, dtype=bool), axis=0),   #可刪
    ...
    def set_initial_data(self, job_length_list, op_pt_list):
        ...
        # [FIX] Directly create the candidate mask from the ground-truth relation matrix.
        # This avoids potential float precision issues or other desync bugs from the indirect `candidate_pt == 0` method.
        env_idx_for_lookup = np.arange(self.number_of_envs)[:, np.newaxis]
        self.candidate_process_relation = self.reverse_process_relation[env_idx_for_lookup, self.candidate]
        ...
        # First, create the state object that rebuild_state_from_current() expects to exist.
        self.state = copy.deepcopy(self.old_state)

        # Now, call rebuild_state_from_current(). This will run the full calculation logic
        # and correctly update the self.state object with the final, consistent features and masks.
        self.rebuild_state_from_current()

        # After the state is fully computed, save it back to old_state for future resets.
        ...
        # self.state is now correct and ready to be returned.
        return self.state
    ...
    def reset(self):
        ...
        # [FIX] Re-run the full state computation to ensure consistency after reset.
        # This recalculates dynamic masks and features based on the reset variables.
        self.rebuild_state_from_current()
    ...
    def initial_vars(self):
        ...
        self.true_op_st = np.full((self.number_of_envs, self.number_of_ops), -1, dtype=float)
    ...
    def step(self, actions):
        ...
        print("*"*50)
        print(chosen_op)
        # =================== MASK INCONSISTENCY DEBUG BLOCK (INSIDE ENV) ===================
        # Check for invalid actions before the environment crashes
        invalid_mask_for_crash_check = self.reverse_process_relation[self.env_idxs, chosen_op, chosen_mch]
        if invalid_mask_for_crash_check.any():
            # Find the first environment that failed
            crashing_env_idx = np.where(invalid_mask_for_crash_check)[0][0]
            
            # Get the specific job, mch, and op for that environment
            crashing_action = actions[crashing_env_idx]
            crashing_job = crashing_action // self.number_of_machines
            crashing_mch = crashing_action % self.number_of_machines
            crashing_op = self.candidate[crashing_env_idx, crashing_job]

            print("="*80)
            print(f"!! PRE-CRASH REPORT at step: {self.step_count}, for env: {crashing_env_idx} !!")
            print(f"   Action chosen by agent: {crashing_action} (Job {crashing_job}, Mch {crashing_mch})")
            print(f"   This corresponds to Operation: {crashing_op}")
            print("-"*80)
            
            # self.dynamic_pair_mask currently holds the mask for the *next* state.
            # The agent made its decision based on the mask from the *previous* state.
            # This mask is stored in self.old_state, which was copied to self.state at the start of the step.
            # Let's assume the state passed to the agent was self.state when it was called.
            # The mask the agent used is therefore in the a copy of the env, not easy to get here.
            # BUT, we can check the ground truth of the *current* candidates.
            # self.candidate_process_relation was updated for the *next* candidate in the previous step.
            
            # The agent made a decision based on the `dynamic_pair_mask` returned from the PREVIOUS step.
            # Let's check the current ground truth for the op that was a candidate in the previous step.
            # This is difficult without storing the previous mask.
            # Let's just print the crucial facts we have now.
            
            ground_truth_is_invalid = self.reverse_process_relation[crashing_env_idx, crashing_op, crashing_mch]

            print(f"CRASH ANALYSIS:")
            print(f"  - The environment is about to crash because for Op {crashing_op} and Mch {crashing_mch},")
            print(f"  - the ground truth `reverse_process_relation` is: {ground_truth_is_invalid} (True means INVALID).")
            print(f"  - This means the agent should NOT have chosen this action.")
            print(f"  - The fact that it DID choose this action proves the `dynamic_pair_mask` it received in the previous step was WRONG.")
            print(f"  - Specifically, for Job {crashing_job} and Mch {crashing_mch}, the mask must have been `False` (VALID).")
            print("="*80)

        # The old check `if (self.reverse_process_relation...).any()` was unreliable because
        # self.reverse_process_relation is calculated only once and can become stale.
        # This new check uses the `self.op_pt` masked array directly, which holds the current ground truth.
        # An operation is invalid if its processing time on a machine is masked (non-existent).
        # Note: In this project, a PT of 0 is also considered invalid, which is handled by the mask.
        chosen_pts = self.op_pt[self.env_idxs, chosen_op, chosen_mch]
        if np.ma.is_masked(chosen_pts):
            # Find the first invalid action to report it
            invalid_indices = np.where(chosen_pts.mask)
            first_invalid_env_idx = self.env_idxs[invalid_indices[0][0]]
            
            print(f'FJSP_Env.py Error from choosing action: Invalid action detected for env {first_invalid_env_idx}.')
            print(f'Op {chosen_op} can\'t be processed by Mch {chosen_mch}')
            sys.exit()
        ...
        # Get indices for jobs that have a next operation
        active_indices = np.where(mask_temp)[0]

        if active_indices.size > 0:
            job_idx_active = chosen_job[active_indices]
            op_idx_active = chosen_op[active_indices]

            self.candidate_pt[active_indices, job_idx_active] = self.unmasked_op_pt[active_indices, op_idx_active + 1]
            self.candidate_process_relation[active_indices, job_idx_active] = \
                self.reverse_process_relation[active_indices, op_idx_active + 1]

        # Get indices for jobs that are now finished
        finished_mask = ~mask_temp
        finished_indices = np.where(finished_mask)[0]

        if finished_indices.size > 0:
            job_idx_finished = chosen_job[finished_indices]
            self.candidate_process_relation[finished_indices, job_idx_finished] = 1
    ...
    def norm_op_features(self):
        ...
        mean_fea_j = np.sum(self.fea_j, axis=1) / (num_left_nodes + 1e-8)
        temp = np.where(self.delete_mask_fea_j, mean_fea_j[:, np.newaxis, :], self.fea_j)
        var_fea_j = np.var(temp, axis=1)

        std_fea_j = np.sqrt(var_fea_j * self.number_of_ops / (num_left_nodes + 1e-8))
    ...
    def norm_machine_features(self):
        ...
        mean_fea_m = np.sum(self.fea_m, axis=1) / (num_left_mchs + 1e-8)
        temp = np.where(self.delete_mask_fea_m,
                        mean_fea_m[:, np.newaxis, :], self.fea_m)
        var_fea_m = np.var(temp, axis=1)
        std_fea_m = np.sqrt(var_fea_m * self.number_of_machines / (num_left_mchs + 1e-8))
    ...
    def _recompute_after_time_override(self):
        ...
        此版本【嚴格】使用「正規化」時間系統，以匹配 step() 和 construct_mch_features() 的邏輯。
        """
        # ----- next_schedule_time 依據 pair_free_time 重新計算 -----
        
        # [FIXED] 確保 pair_free_time 是用「正規化」時間計算的
        ...
        # [FIXED] self.pair_free_time 儲存「正規化時間」
        ...
        self.op_waiting_time[self.env_job_idx, self.candidate] = \
            (1 - self.mask) * np.maximum(
                # [FIXED] 確保使用「正規化」的 candidate_free_time
        ...
        # [FIXED] 確保使用「正規化」的 mch_free_time
        ...
        # [REMOVED] L669 (self.mch_free_time = np.copy(self.true_mch_free_time))
        # 移除這行，防止「真值」汙染「正規化」的 mch_free_time

        # 重新組裝特徵（這三個會內部做正規化處理）
        # 這裡會正確呼叫 L546 的 construct_mch_features，
        # 它會讀取「正規化」的 mch_free_time (e.g., 0.0625)
        self.construct_op_features()
        self.construct_mch_features()
        self.construct_pair_features()
    ...
    def rebuild_state_from_current(self):
        ...
        用於重排：當 orchestrator 覆寫了 ready/machine 可用時間後，呼叫本函式以：
          1) 重算 next_schedule_time / 動態遮罩 / 各種特徵
