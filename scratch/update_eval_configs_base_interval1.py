    ...
    for filename in os.listdir(yaml_config_dir):
        if filename.startswith("eval_baseline_seed") and filename.endswith(".yml"):
            ...
            for line in lines:
                ...
                if line.strip().startswith("hl_ppo_model_path:"):
                    updated_lines.append("hl_ppo_model_path: trained_weights\\high_level\\hlgate_scn_base_newstate_s07_p15_e8.pth\n")
                # Check for decision interval
                elif line.strip().startswith("hl_gate_decision_interval:"):
                    updated_lines.append("hl_gate_decision_interval: 1\n")
                else:
