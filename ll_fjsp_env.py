...
class LLFJSPEnv:
    ...
    def _compute_pair_free_time(self, env_indices=None, true_time=False):
        ...
        if not self.enable_gap_insertion:
            # Vectorized implementation bypassing all nested loops
            candidate_free_sub = candidate_free[env_indices, :]
            release_time_sub = release_time[env_indices, :]
            earliest = np.maximum(candidate_free_sub, release_time_sub)
            
            earliest_expanded = earliest[:, :, np.newaxis]
            fallback_mch_free_sub = fallback_mch_free[env_indices, :]
            fallback_mch_free_expanded = fallback_mch_free_sub[:, np.newaxis, :]
            
            val = np.maximum(earliest_expanded, fallback_mch_free_expanded)
            mask_sub = self.candidate_process_relation[env_indices, :, :]
            out = np.where(mask_sub, 0.0, val)
            return out
        else:
            # Fallback to standard nested loops for gap insertion
    ...
    def set_initial_data(self, job_length_list, op_pt_list, due_date_list=None, normalize_due_date=True, true_due_date_list=None, tightness=None, release_time_list=None):
        ...
        self.overdue_job_progress_ct = np.zeros((self.number_of_envs, self.number_of_jobs), dtype=np.float64)
    ...
    def step(self, actions):
        ...
        if not self.enable_gap_insertion:
            ...
            chosen_op_st = np.maximum(earliest, self.mch_free_time[self.incomplete_env_idx, chosen_mch])
        else:
        ...
        if not self.enable_gap_insertion:
            ...
            true_chosen_op_st = np.maximum(earliest_true, self.true_mch_free_time[self.incomplete_env_idx, chosen_mch])
        else:
        ...
        safe_next_op = np.where(candidate_add_flag, chosen_op + 1, 0)
        self.candidate_pt[self.incomplete_env_idx, chosen_job] = np.where(
            candidate_add_flag[:, np.newaxis],
            self.unmasked_op_pt[self.incomplete_env_idx, safe_next_op],
            self.candidate_pt[self.incomplete_env_idx, chosen_job]
        )
        self.candidate_process_relation[self.incomplete_env_idx, chosen_job] = np.where(
            candidate_add_flag[:, np.newaxis],
            self.reverse_process_relation[self.incomplete_env_idx, safe_next_op],
            True
        )
        ...
        self.op_assigned_mch[self.incomplete_env_idx, chosen_op] = chosen_mch
        ...
        op_job_idx = np.repeat(np.arange(self.number_of_jobs), self.virtual_job_length[0])
        op_job_idx_expanded = op_job_idx[np.newaxis, :]
        target_job_mask = (op_job_idx_expanded == chosen_job[:, np.newaxis])
        
        self.op_match_job_left_op_nums[self.incomplete_env_idx] -= target_job_mask
        self.op_match_job_remain_work[self.incomplete_env_idx] -= target_job_mask * self.op_mean_pt[self.incomplete_env_idx, chosen_op][:, np.newaxis]
        self.true_op_match_job_remain_work[self.incomplete_env_idx] -= target_job_mask * self.true_op_mean_pt[self.incomplete_env_idx, chosen_op][:, np.newaxis]
        
        op_idx = np.arange(self.max_number_of_ops)[np.newaxis, :]
        lb_mask = target_job_mask & (op_idx >= chosen_op[:, np.newaxis])
        self.op_ct_lb[self.incomplete_env_idx] += lb_mask * diff[:, np.newaxis]
        ...
        self.unavailable_pairs = pair_free_time > self.next_schedule_time[self.incomplete_env_idx][:, np.newaxis, np.newaxis]
        ...
        od_coef = float(getattr(configs, "ll_overdue_progress_coef", 0.0))
        reward_od = np.zeros(self.number_of_envs, dtype=np.float64)
        if od_coef != 0.0:
            base_scale = max(float(self.mean_op_pt), 1e-6)
            prev_progress = self.overdue_job_progress_ct[self.incomplete_env_idx, chosen_job]
            new_progress = np.maximum(prev_progress, relevant_completion_times)
            progress_delta = np.maximum(0.0, new_progress - prev_progress)
            overdue_weight = np.log1p(np.maximum(0.0, -relevant_due_dates) / base_scale)
            reward_od_local = -(overdue_weight * progress_delta / base_scale)
            reward_od[self.incomplete_env_idx] = reward_od_local
            self.overdue_job_progress_ct[self.incomplete_env_idx, chosen_job] = new_progress

        wait_od_coef = float(getattr(configs, "ll_ready_overdue_wait_coef", 0.0))
        reward_wait_od = np.zeros(self.number_of_envs, dtype=np.float64)
        if wait_od_coef != 0.0:
            threshold = max(0.0, float(getattr(configs, "ll_ready_overdue_wait_threshold", 0.0)))
            base_scale = max(float(self.mean_op_pt), 1e-6)
            active_due = self.true_due_date[self.incomplete_env_idx]
            current_time = self.true_candidate_free_time[self.incomplete_env_idx, chosen_job]
            ready_mask = (
                (~self.mask[self.incomplete_env_idx])
                & (self.true_candidate_free_time[self.incomplete_env_idx] <= current_time[:, np.newaxis] + 1e-9)
            )
            ready_overdue_weight = np.log1p(np.maximum(0.0, -active_due) / base_scale)
            ready_overdue_weight = np.where(ready_mask, ready_overdue_weight, 0.0)
            max_ready_overdue = np.max(ready_overdue_weight, axis=1)
            chosen_overdue = ready_overdue_weight[np.arange(len(chosen_job)), chosen_job]
            wait_penalty = np.maximum(0.0, max_ready_overdue - chosen_overdue)
            wait_penalty = np.where(max_ready_overdue >= threshold, wait_penalty, 0.0)
            reward_wait_od[self.incomplete_env_idx] = -wait_penalty
    ...
    def construct_op_features(self):
        ...
        # Expand due_date to op dimension: [E, N] using np.repeat along axis=1
        op_due_date = np.repeat(self.due_date, self.virtual_job_length[0], axis=1)
        ...
        feat_job_current_tardiness = np.repeat(self._log1p_scaled(job_current_tardiness), self.virtual_job_length[0], axis=1)
        ...
        # Vectorized slack rank computation
        active_count = np.sum(~self.mask, axis=1)
        denom = np.maximum(active_count - 1, 1)[:, np.newaxis]
        slack_masked = np.where(self.mask, np.inf, job_slack)
        ranks = np.argsort(np.argsort(slack_masked, axis=1), axis=1).astype(np.float32)
        job_slack_rank = np.where(self.mask, 1.0, ranks / denom)
        
        feat_slack_rank = np.repeat(job_slack_rank, self.virtual_job_length[0], axis=1)
        ...
        feat_slack_gap_to_min = np.repeat(np.log1p(job_slack_gap_to_min), self.virtual_job_length[0], axis=1)

        op_flex_counts = self.compatible_op.astype(np.float32)
        remaining_flex_min = np.ones((self.number_of_envs, self.number_of_jobs), dtype=np.float32)
        remaining_flex_mean = np.ones((self.number_of_envs, self.number_of_jobs), dtype=np.float32)
        
        # Build mappings from op to job
        op_job_idx = np.repeat(np.arange(self.number_of_jobs), self.virtual_job_length[0])
        op_job_idx_expanded = op_job_idx[np.newaxis, :]
        
        # Determine valid remaining ops per env
        current_candidate = self.candidate[np.arange(self.number_of_envs)[:, np.newaxis], op_job_idx_expanded]
        current_last_op = self.job_last_op_id[np.arange(self.number_of_envs)[:, np.newaxis], op_job_idx_expanded]
        current_mask = self.mask[np.arange(self.number_of_envs)[:, np.newaxis], op_job_idx_expanded]
        
        op_idx = np.arange(self.max_number_of_ops)[np.newaxis, :]
        valid_remaining = (op_idx >= current_candidate + 1) & (op_idx <= current_last_op) & (~current_mask)
        
        op_flex = op_flex_counts / max(float(self.number_of_machines), 1.0)
        
        indices = self.job_first_op_id[0].astype(np.int32)
        
        # Min flex
        flex_for_min = np.where(valid_remaining, op_flex, 1.0)
        remaining_flex_min = np.minimum.reduceat(flex_for_min, indices, axis=1)
        
        # Mean flex
        flex_for_sum = np.where(valid_remaining, op_flex, 0.0)
        sum_j = np.add.reduceat(flex_for_sum, indices, axis=1)
        count_j = np.add.reduceat(valid_remaining.astype(np.int32), indices, axis=1)
        remaining_flex_mean = np.where(count_j > 0, sum_j / np.maximum(count_j, 1), 1.0)
            
        feat_remaining_flex_min = np.repeat(remaining_flex_min, self.virtual_job_length[0], axis=1)
        feat_remaining_flex_mean = np.repeat(remaining_flex_mean, self.virtual_job_length[0], axis=1)
